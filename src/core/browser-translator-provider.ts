...
const isTransientFailure = (error: unknown) => (
  ...
  error instanceof Error && error.message.includes('Other generic failures occurred')
);
...
const wait = (delayMs: number, signal?: AbortSignal) => new Promise<void>((resolve, reject) => {
  ...
  const onAbort = () => {
    ...
    reject(signal?.reason ?? new DOMException('Translation aborted', 'AbortError'));
  };
        ...
        try {
          for (const text of texts) {
            ...
            if (!translated) {
              ...
              if (!translated) throw new Error(`Translation failed after ${maxAttempts} attempts.`);
