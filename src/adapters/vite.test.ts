...
describe('viteSourceUpdates', () => {
  it('is safe outside Vite', () => {
    ...
    expect(() => viteSourceUpdates(undefined).subscribe(onUpdate)()).not.toThrow();
    expect(onUpdate).not.toHaveBeenCalled();
  });

  it('notifies after matching Vite updates and unsubscribes', () => {
    let listener: ((payload: { updates: Array<{ path: string }> }) => void) | undefined;
    const hot = {
      on: vi.fn((_event, nextListener) => { listener = nextListener; }),
      off: vi.fn(),
    };
    const onUpdate = vi.fn();
    const unsubscribe = viteSourceUpdates(hot, { watchPaths: ['src/emails'] }).subscribe(onUpdate);
