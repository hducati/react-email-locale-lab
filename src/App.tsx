    ...
    <main className="email-lab">
      <header className="topbar">
        <div className="topbar-copy">
          <div className="brand">
            ...
            <p className="eyebrow">React Email Locale Lab</p>
          </div>
        ...
        <div className="controls">
          <label>
            Template
            <select
              value={lab.selectedTemplateId}
              onChange={(event) => lab.selectTemplate(event.target.value)}
            >
              {lab.templateIds.map((id) => (
                <option key={id} value={id}>
                  {config.templates[id].name}
                </option>
              ))}
            </select>
          </label>
      ...
      {lab.activeLocales.length === 0 && (
        <section className="empty-state">
          ...
          <span>
            The source template renders immediately; translation starts only on
            demand.
          </span>
        </section>
      )}
